...
class SMPLify3D():
    ...
    def __init__(
        ...
        self.pose_prior = MaxMixturePrior(
            prior_folder=config.GMM_MODEL_DIR,
            num_gaussians=8,
            dtype=torch.float32
        ).to(device)
        # collision part
        ...
        if self.use_collision:
            self.part_segm_fn = config.PART_SEG_FN
        
        # reLoad SMPL-X model
        self.smpl = smplxmodel

        # self.model_faces = smplxmodel.faces_tensor.view(-1)
        self.model_faces = smplxfaces.view(-1)
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        ...
        model_joints = smpl_output["joints_wotrans"][:, :22, :]
        body_pose = init_pose[:, 1:22, :].detach().clone()
        hand_pose = init_pose[:, 22:, :].detach().clone()
        global_orient = init_pose[:, :1, :].detach().clone()
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        if self.use_lbfgs:
            ...
            for i in range(10):
                def closure():
                    ...
                    smpl_output = self.smpl({
                        "rot6d": torch.cat([global_orient, body_pose, hand_pose], dim=1),
        ...
        if self.use_lbfgs:
            ...
            for i in range(self.num_iters):
                def closure():
                    ...
                    model_joints = smpl_output["joints_wotrans"][:, :22, :]
                    model_vertices = smpl_output["vertices_wotrans"]

                    loss = body_fitting_loss_3d(
                        # torch.cat([
                        #     rotation_matrix_to_angle_axis(rot6d_to_rotation_matrix(body_pose)).squeeze().contiguous().view(1, 63),
                        #     torch.zeros(1, 6).to(body_pose.device)
                        # ], dim=-1),
                        body_pose,
                        preserve_pose,
                        betas,
        ...
        pose = torch.cat([global_orient, body_pose, hand_pose], dim=1).detach()
        betas = betas.detach()
